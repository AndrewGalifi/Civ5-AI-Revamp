	...
	if(bFirstTime)
	{
		...
		for(list<int>::iterator it = m_CurrentTurnUnits.begin(); it != m_CurrentTurnUnits.end(); ++it)
		{
			...
			if(pLoopUnit && !pLoopUnit->isHuman())
			{
				...
				//MOD: keep the experiment player's dedicated explorer from being pulled into homeland military jobs.
				if(pLoopUnit->AI_getUnitAIType() == UNITAI_EXPLORE && (ShouldUseStrategyDirectiveAI(m_pPlayer->GetID()) || m_pPlayer->GetEconomicAI()->GetReconState() != RECON_STATE_ENOUGH))
				{
					continue;
				}
