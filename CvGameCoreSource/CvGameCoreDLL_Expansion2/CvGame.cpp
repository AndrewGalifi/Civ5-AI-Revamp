...
void CvGame::setWinner(TeamTypes eNewWinner, VictoryTypes eNewVictory)
{
	if((getWinner() != eNewWinner) || (getVictory() != eNewVictory))
	{
		...
		//MOD: log official experiment result when the game engine declares a winner
		if(eNewWinner != NO_TEAM && eNewVictory != NO_VICTORY)
		{
			const PlayerTypes eExperimentPlayer = (PlayerTypes)1;
			CvPlayerAI& kExperimentPlayer = GET_PLAYER(eExperimentPlayer);
			const PlayerTypes eWinningLeader = GET_TEAM(eNewWinner).getLeaderID();
			CvPlayerAI& kWinningLeader = GET_PLAYER(eWinningLeader);

			int iExperimentProduction = 0;
			int iCityLoop = 0;
			for(CvCity* pCity = kExperimentPlayer.firstCity(&iCityLoop); pCity != NULL; pCity = kExperimentPlayer.nextCity(&iCityLoop))
			{
				iExperimentProduction += pCity->getYieldRate(YIELD_PRODUCTION, false);
			}

			int iAliveMajors = 0;
			for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
			{
				CvPlayerAI& kLoopPlayer = GET_PLAYER((PlayerTypes)iPlayerLoop);
			...
			const CvVictoryInfo* pkVictoryInfo = GC.getVictoryInfo(eNewVictory);
			CvString strHeader = "Turn,WinnerTeam,WinnerLeader,WinnerCiv,VictoryType,ExperimentPlayer,ExperimentTeam,ExperimentAlive,ExperimentWon,ExperimentScore,ExperimentCities,ExperimentPopulation,ExperimentScience,ExperimentTechs,ExperimentPolicies,ExperimentProduction,ExperimentMilitary,ExperimentGold,ExperimentGPTx100,ExperimentHappiness,AliveMajors";
			CvString strOutBuf;
			strOutBuf.Format("%03d,%d,%d,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
				getElapsedGameTurns(),
				(int)eNewWinner,
				(int)eWinningLeader,
				kWinningLeader.getCivilizationShortDescription(),
				pkVictoryInfo != NULL ? pkVictoryInfo->GetType() : "NO_VICTORY_INFO",
				(int)eExperimentPlayer,
				(int)kExperimentPlayer.getTeam(),
				kExperimentPlayer.isAlive() ? 1 : 0,
				kExperimentPlayer.getTeam() == eNewWinner ? 1 : 0,
				kExperimentPlayer.GetScore(),
				kExperimentPlayer.getNumCities(),
				kExperimentPlayer.getTotalPopulation(),
				kExperimentPlayer.GetScience(),
				GET_TEAM(kExperimentPlayer.getTeam()).GetTeamTechs()->GetNumTechsKnown(),
				kExperimentPlayer.GetPlayerPolicies() != NULL ? kExperimentPlayer.GetPlayerPolicies()->GetNumPoliciesOwned() : 0,
				iExperimentProduction,
				kExperimentPlayer.GetMilitaryMight(),
				kExperimentPlayer.GetTreasury() != NULL ? kExperimentPlayer.GetTreasury()->GetGold() : 0,
				kExperimentPlayer.GetTreasury() != NULL ? kExperimentPlayer.GetTreasury()->CalculateBaseNetGoldTimes100() : 0,
				kExperimentPlayer.GetExcessHappiness(),
				iAliveMajors);

			FILogFile* pExperimentResultLog = LOGFILEMGR.GetLog("StrategyDirectiveAI_Result.csv", FILogFile::kDontTimeStamp, strHeader);
			pExperimentResultLog->Msg(strOutBuf);
		}
		//END MOD
