...
//MOD: reject remote city sites for the experiment player
static bool IsExperimentSettlePlotWithinCityRange(const CvPlayerAI* pPlayer, const CvPlot* pPlot)
{
	if(pPlayer == NULL || pPlot == NULL || !ShouldUseStrategyDirectiveAI(pPlayer->GetID()))
	{
		return true;
	}
	...
	{
		bHasCity = true;
		const int iPathDistance = GC.getStepFinder().GetStepDistanceBetweenPoints(pPlayer->GetID(), NO_PLAYER, pLoopCity->plot(), const_cast<CvPlot*>(pPlot));
...
{
	if(pPlayer == NULL || !ShouldUseStrategyDirectiveAI(pPlayer->GetID()) || GC.getGame().getGameTurn() < StrategyDirectiveAIConstants::NC_TECH_PATH_PRIORITY_TURN || IsExperimentNationalCollegeBuilt(pPlayer))
	{
		return NO_TECH;
	}

	const TechTypes ePhilosophy = (TechTypes)GC.getInfoTypeForString("TECH_PHILOSOPHY", true);
	if(ePhilosophy == NO_TECH || GET_TEAM(pPlayer->getTeam()).GetTeamTechs()->HasTech(ePhilosophy))
	{
		return NO_TECH;
	}

	const TechTypes aeNationalCollegePath[] =
	{
		(TechTypes)GC.getInfoTypeForString("TECH_PHILOSOPHY", true),
		(TechTypes)GC.getInfoTypeForString("TECH_WRITING", true),
		(TechTypes)GC.getInfoTypeForString("TECH_POTTERY", true)
	};

	for(int iI = 0; iI < 3; iI++)
	{
		const TechTypes eTech = aeNationalCollegePath[iI];
		if(eTech != NO_TECH && pPlayer->GetPlayerTechs()->CanResearch(eTech))
		{
			return eTech;
		}
	}
...
void CvPlayerAI::AI_chooseResearch()
{
	...
	if(GetPlayerTechs()->GetCurrentResearch() == NO_TECH)
	{
		...
		//MOD: prioritize the Philosophy path while National College is still unresolved
		if(eBestTech == NO_TECH)
		{
			eBestTech = GetExperimentNationalCollegePathTech(this);
		}
