	...
	*bRequired = true;
	...
	if(iThisFormationIndex != NO_MUFORMATION)
	{
		...
		if(thisFormation)
		{
			...
			//MOD: let settler escorts interrupt tactical assignments for the experiment player
			const bool bPrioritizeSettlerEscort = ShouldUseStrategyDirectiveAI(m_eOwner) && GetOperationType() == AI_OPERATION_FOUND_CITY && thisOperationSlot.m_iSlotID == 1;
			...
			for(CvUnit* pLoopUnit = ownerPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = ownerPlayer.nextUnit(&iLoop))
			{
				...
				if((pLoopUnit->canRecruitFromTacticalAI() || bPrioritizeSettlerEscort) && pLoopUnit->getArmyID() == FFreeList::INVALID_INDEX &&
				        eLoopUnitAIType != UNITAI_EXPLORE && eLoopUnitAIType != UNITAI_EXPLORE_SEA && pLoopUnit->getDropRange() == 0 /* no paratroopers */)
				{
					...
					if(unitInfo->GetUnitAIType((UnitAITypes)thisSlotEntry.m_primaryUnitType))
					{
						...
						if(!pLoopUnit->IsCombatUnit() || pLoopUnit->GetCurrHitPoints() >= pLoopUnit->GetMaxHitPoints() * GC.getAI_OPERATIONAL_PERCENT_HEALTH_FOR_OPERATION() / 100)
						{
							...
							if(pLoopUnit->GetDeployFromOperationTurn() + GC.getAI_TACTICAL_MAP_TEMP_ZONE_TURNS() < GC.getGame().getGameTurn() || bPrioritizeSettlerEscort)
							{
			...
			if(pBestUnit != NULL)
			{
				if(bPrioritizeSettlerEscort)
				{
					pBestUnit->setTacticalMove(NO_TACTICAL_MOVE);
				}
