...
static bool IsExperimentSettlePlotWithinCityRange(const CvPlayer* pPlayer, const CvPlot* pPlot)
{
...
bool CvPlayer::canFound(int iX, int iY, bool bTestVisible) const
{
	...
	//MOD: hard-freeze experiment city founding while National College is pending.
...
CvPlot* CvPlayer::GetBestSettlePlot(CvUnit* pUnit, bool bEscorted, int iArea) const
{
	...
	for(int iPlotLoop = 0; iPlotLoop < iNumPlots; iPlotLoop++)
	{
		...
		//MOD: do not send experiment settlers to found while National College is pending.
		if(IsExperimentNationalCollegePendingForSettling(this))
		{
			continue;
		}

		//MOD: do not send experiment settlers to remote city sites.
