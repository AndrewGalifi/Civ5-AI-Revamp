...
void CvEconomicAI::DoReconState()
{
	...
	//MOD: cap early land recon and return only normal combat units
	if(ShouldUseStrategyDirectiveAI(m_pPlayer->GetID()) && GC.getGame().getGameTurn() >= 9 && m_pPlayer->GetNumUnitsWithUnitAI(UNITAI_EXPLORE, true, false) >= 1)
	{
		m_eReconState = RECON_STATE_ENOUGH;
		...
		for(pLoopUnit = m_pPlayer->firstUnit(&iUnitLoop); pLoopUnit != NULL; pLoopUnit = m_pPlayer->nextUnit(&iUnitLoop))
		{
			...
			{
				UnitAITypes eNewUnitAI = NO_UNITAI;
				if(pLoopUnit->getUnitInfo().GetUnitAIType(UNITAI_ATTACK))
				{
					eNewUnitAI = UNITAI_ATTACK;
				}
				else if(pLoopUnit->getUnitInfo().GetUnitAIType(UNITAI_COUNTER))
				{
					eNewUnitAI = UNITAI_COUNTER;
				}
				else if(pLoopUnit->getUnitInfo().GetUnitAIType(UNITAI_DEFENSE))
				{
					eNewUnitAI = UNITAI_DEFENSE;
				}
				else if(pLoopUnit->getUnitInfo().GetUnitAIType(UNITAI_RANGED))
				{
					eNewUnitAI = UNITAI_RANGED;
				}

				if(eNewUnitAI != NO_UNITAI)
				{
					pLoopUnit->AI_setUnitAIType(eNewUnitAI);
					if(GC.getLogging() && GC.getAILogging())
					{
						CvString strLogString;
						strLogString.Format("StrategyDirectiveAI returning exploring %s to military AI %d, X: %d, Y: %d", pLoopUnit->getName().GetCString(), (int)eNewUnitAI, pLoopUnit->getX(), pLoopUnit->getY());
						m_pPlayer->GetHomelandAI()->LogHomelandMessage(strLogString);
					}
				}
			}
