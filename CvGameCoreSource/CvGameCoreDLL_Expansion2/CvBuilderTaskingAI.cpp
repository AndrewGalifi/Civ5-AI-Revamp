...
//MOD: builder tile-improvement priority nudges for experiment player
namespace
{
	...
	bool IsExperimentBuilderPriorityPlayer(CvPlayer* pPlayer)
	{
	...
	bool IsExperimentNearCivilService(CvPlayer* pPlayer)
	{
		...
		return pPlayerTechs != NULL && pPlayerTechs->GetCurrentResearch() == eCivilService && pPlayerTechs->GetResearchTurnsLeft(eCivilService, true) <= AI_EXPERIMENT_CIVIL_SERVICE_FARM_TURNS;
	}


	int GetExperimentBuilderPriorityWeight(CvPlayer* pPlayer, CvPlot* pPlot, CvBuildInfo* pkBuild, ImprovementTypes eImprovement, ResourceTypes eResource, int iFoodDelta, int iProductionDelta, int iWeight)
	{
		if(!IsExperimentBuilderPriorityPlayer(pPlayer) || pPlot == NULL || pkBuild == NULL || iWeight <= 0)
		{
			return iWeight;
		}
		...
		else if(iProductionDelta > 0)
		{
			iPriorityBand = max(iPriorityBand, AI_EXPERIMENT_PRODUCTION_IMPROVEMENT_BAND + min(iProductionDelta, 3));
		}
		else if(iFoodDelta > 0)
		{
			iPriorityBand = max(iPriorityBand, AI_EXPERIMENT_FOOD_IMPROVEMENT_BAND + min(iFoodDelta, 3));
		}
		...
		return CorrectWeight(iPriorityBand + max(1, iWeight / AI_EXPERIMENT_PRIORITY_TIEBREAKER_DIVISOR));
	}
}
//END MOD
...
void CvBuilderTaskingAI::AddImprovingResourcesDirectives(CvUnit* pUnit, CvPlot* pPlot, int iMoveTurnsAway)
{
	...
	for(iBuildIndex = 0; iBuildIndex < GC.getNumBuildInfos(); iBuildIndex++)
	{
		...
		//MOD: resource improvements, especially luxuries, should win early worker attention.
		iWeight = GetExperimentBuilderPriorityWeight(m_pPlayer, pPlot, pkBuild, eImprovement, eResource, m_aiProjectedPlotYields[YIELD_FOOD] - m_aiCurrentPlotYields[YIELD_FOOD], m_aiProjectedPlotYields[YIELD_PRODUCTION] - m_aiCurrentPlotYields[YIELD_PRODUCTION], iWeight);
		if(iWeight <= 0)
		{
			continue;
		}
...
void CvBuilderTaskingAI::AddImprovingPlotsDirectives(CvUnit* pUnit, CvPlot* pPlot, int iMoveTurnsAway)
{
	...
	for(iBuildIndex = 0; iBuildIndex < GC.getNumBuildInfos(); iBuildIndex++)
	{
		...
		//MOD: normal tile ordering: production gains before food, with a Civil Service farm setup boost.
		iWeight = GetExperimentBuilderPriorityWeight(m_pPlayer, pPlot, pkBuild, eImprovement, eResource, m_aiProjectedPlotYields[YIELD_FOOD] - m_aiCurrentPlotYields[YIELD_FOOD], m_aiProjectedPlotYields[YIELD_PRODUCTION] - m_aiCurrentPlotYields[YIELD_PRODUCTION], iWeight);
		BuilderDirective directive;
		directive.m_eDirective = eDirectiveType;
		directive.m_eBuild = eBuild;
