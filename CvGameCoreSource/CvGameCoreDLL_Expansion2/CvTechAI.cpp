...
//MOD: experiment soft priority tech path nudges.
namespace
{
	const int AI_EXPERIMENT_PRIORITY_TECH_PATH_WEIGHT_BONUS = 120;
	const int AI_EXPERIMENT_PRIORITY_TECH_DIRECT_WEIGHT_BONUS = 180;
	const int AI_EXPERIMENT_SCIENCE_MILESTONE_PATH_WEIGHT_BONUS = 260;
	const int AI_EXPERIMENT_SCIENCE_MILESTONE_DIRECT_WEIGHT_BONUS = 380;
	const int AI_EXPERIMENT_PRIORITY_TECH_MAX_RECURSION = 12;

	const char* AI_EXPERIMENT_SCIENCE_MILESTONE_TECHS[] =
	{
		"TECH_PHILOSOPHY",
		"TECH_EDUCATION",
	...
	const char* AI_EXPERIMENT_PRIORITY_TECHS[] =
	{
		"TECH_POTTERY",
	...
	bool IsExperimentPriorityTechPlayer(CvPlayer* pPlayer)
	{
		return pPlayer != NULL && ShouldUseStrategyDirectiveAI(pPlayer->GetID());
	}

	bool IsExperimentTechKnown(CvPlayer* pPlayer, TechTypes eTech)
	{
		return pPlayer != NULL && eTech != NO_TECH && GET_TEAM(pPlayer->getTeam()).GetTeamTechs()->HasTech(eTech);
	}

	bool IsExperimentTechOnPriorityPath(CvPlayer* pPlayer, TechTypes eCandidate, TechTypes eTarget, int iDepth)
	{
		if(eCandidate == NO_TECH || eTarget == NO_TECH || iDepth > AI_EXPERIMENT_PRIORITY_TECH_MAX_RECURSION)
		{
			return false;
		}

		if(eCandidate == eTarget)
		{
			return true;
		}

		CvTechEntry* pkTargetTech = GC.getTechInfo(eTarget);
		if(pkTargetTech == NULL)
		{
			return false;
		}
		...
		{
			const TechTypes ePrereq = (TechTypes)pkTargetTech->GetPrereqAndTechs(iI);
			if(ePrereq != NO_TECH && !IsExperimentTechKnown(pPlayer, ePrereq) && IsExperimentTechOnPriorityPath(pPlayer, eCandidate, ePrereq, iDepth + 1))
			{
				return true;
			}
		}
		...
		{
			const TechTypes ePrereq = (TechTypes)pkTargetTech->GetPrereqOrTechs(iI);
			if(ePrereq != NO_TECH && !IsExperimentTechKnown(pPlayer, ePrereq) && IsExperimentTechOnPriorityPath(pPlayer, eCandidate, ePrereq, iDepth + 1))
			{
				return true;
			}
	...
	TechTypes GetExperimentCurrentScienceMilestoneTech(CvPlayer* pPlayer)
	{
		for(unsigned int iI = 0; iI < sizeof(AI_EXPERIMENT_SCIENCE_MILESTONE_TECHS) / sizeof(AI_EXPERIMENT_SCIENCE_MILESTONE_TECHS[0]); iI++)
		{
			const TechTypes eTarget = (TechTypes)GC.getInfoTypeForString(AI_EXPERIMENT_SCIENCE_MILESTONE_TECHS[iI], true);
			if(eTarget != NO_TECH && !IsExperimentTechKnown(pPlayer, eTarget))
			{
				return eTarget;
			}
		}
	...
	int GetExperimentPriorityTechWeight(CvPlayer* pPlayer, TechTypes eCandidate)
	{
		if(!IsExperimentPriorityTechPlayer(pPlayer) || eCandidate == NO_TECH || IsExperimentTechKnown(pPlayer, eCandidate))
		{
			return 0;
		}

		int iBestWeight = 0;
		const TechTypes eScienceMilestone = GetExperimentCurrentScienceMilestoneTech(pPlayer);
		if(eScienceMilestone != NO_TECH)
		{
			if(eCandidate == eScienceMilestone)
			{
				iBestWeight = max(iBestWeight, AI_EXPERIMENT_SCIENCE_MILESTONE_DIRECT_WEIGHT_BONUS);
			}
			else if(IsExperimentTechOnPriorityPath(pPlayer, eCandidate, eScienceMilestone, 0))
			{
				iBestWeight = max(iBestWeight, AI_EXPERIMENT_SCIENCE_MILESTONE_PATH_WEIGHT_BONUS);
			}
		}

		for(unsigned int iI = 0; iI < sizeof(AI_EXPERIMENT_PRIORITY_TECHS) / sizeof(AI_EXPERIMENT_PRIORITY_TECHS[0]); iI++)
		{
			const TechTypes eTarget = (TechTypes)GC.getInfoTypeForString(AI_EXPERIMENT_PRIORITY_TECHS[iI], true);
			if(eTarget == NO_TECH || IsExperimentTechKnown(pPlayer, eTarget))
			{
				continue;
			}
			...
			if(eCandidate == eTarget)
			{
				iBestWeight = max(iBestWeight, AI_EXPERIMENT_PRIORITY_TECH_DIRECT_WEIGHT_BONUS);
			}
			else if(IsExperimentTechOnPriorityPath(pPlayer, eCandidate, eTarget, 0))
			{
				iBestWeight = max(iBestWeight, AI_EXPERIMENT_PRIORITY_TECH_PATH_WEIGHT_BONUS);
			}
		}
	...
	for(iTechLoop = 0; iTechLoop < m_pCurrentTechs->GetTechs()->GetNumTechs(); iTechLoop++)
	{
		if(m_pCurrentTechs->CanResearch((TechTypes)iTechLoop))
		{
			...
			if(bFreeTech)
			{
				...
				{
					m_ResearchableTechs.push_back(iTechLoop, m_TechAIWeights.GetWeight(iTechLoop) + GetExperimentPriorityTechWeight(pPlayer, (TechTypes)iTechLoop));
				}
			}
			else
			{
