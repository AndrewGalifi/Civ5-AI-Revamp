...
void CvCityCitizens::DoTurn()
{
	...
	{
		//MOD: experiment-player citizen focus override
		if(ShouldUseStrategyDirectiveAI(thisPlayer.GetID()))
		{
			...
			if(bForceSettlerProduction)
			{
				SetNoAutoAssignSpecialists(true);
				SetForcedAvoidGrowth(false);
				SetFocusType(CITY_AI_FOCUS_TYPE_PROD_GROWTH);
			}
			else if(bForceEarlyGrowth)
			{
				SetNoAutoAssignSpecialists(true);
				SetForcedAvoidGrowth(false);
			...
			{
				SetNoAutoAssignSpecialists(false);
				SetForcedAvoidGrowth(kDirective.m_bForceAvoidGrowth);
				SetFocusType(kDirective.m_eDefaultCityFocus);
			}
			if(!bDirectiveLocked)
			{
				// Let explicit city roles override the strategic default, but keep weaker heuristics out of the way.
				...
				if(eSpecialization != NO_CITY_SPECIALIZATION && eSpecialization == eWonderSpecializationType)
				{
				...
				else if(eSpecialization != NO_CITY_SPECIALIZATION)
				{
...
int CvCityCitizens::GetPlotValue(CvPlot* pPlot, bool bUseAllowGrowthFlag)
{
	...
	//MOD: experiment player only gets the vanilla low-pop food surge until pop 3, not pop 5
	const int iLowPopulationFoodSurgeThreshold = ShouldUseStrategyDirectiveAI(GetOwner()) ? 3 : 5;
	if((eFocus == NO_CITY_AI_FOCUS_TYPE || eFocus == CITY_AI_FOCUS_TYPE_PROD_GROWTH || eFocus == CITY_AI_FOCUS_TYPE_GOLD_GROWTH) && !bAvoidGrowth && m_pCity->getPopulation() < iLowPopulationFoodSurgeThreshold)
	{
		iFoodYieldValue *= 4;
	}
