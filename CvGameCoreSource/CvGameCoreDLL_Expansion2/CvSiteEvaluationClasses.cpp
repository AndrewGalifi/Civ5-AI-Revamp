...
int CvCitySiteEvaluator::ComputeTradeableResourceValue(CvPlot* pPlot, CvPlayer* pPlayer)
{
	...
	if(eResource != NO_RESOURCE)
	{
		...
		if(eResourceUsage == RESOURCEUSAGE_LUXURY || eResourceUsage == RESOURCEUSAGE_STRATEGIC)
		{
			...
			if(pPlayer)
			{
				//MOD: experiment sites should value new luxuries more decisively.
				if(pPlayer->getNumResourceTotal(eResource) == 0)
				{
					rtnValue *= 3;
					if(ShouldUseStrategyDirectiveAI(pPlayer->GetID()) && eResourceUsage == RESOURCEUSAGE_LUXURY)
					{
						rtnValue += StrategyDirectiveAIConstants::UNIQUE_LUXURY_SETTLE_BONUS;
					}
				}
				//END MOD
			}
