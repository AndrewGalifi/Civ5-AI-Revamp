...
namespace
{
	//MOD: tech flavor bias helper for experiment player
	void AddDirectiveTechFlavorBias(FlavorTypes eTargetFlavor, const char* szFlavorName, int iDelta, int& iFlavorValue)
	{
	...
	int GetDirectiveAdjustedTechFlavorValue(CvPlayer* pPlayer, FlavorTypes eFlavor, int iBaseFlavorValue)
	{
		if(pPlayer == NULL || pPlayer->isMinorCiv() || !ShouldUseStrategyDirectiveAI(pPlayer->GetID()))
		{
		...
		{
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_SCIENCE", 2, iFlavorValue);
		}

		switch(kDirective.m_ePrimaryStrategy)
		{
		case PRIMARY_STRATEGY_DEVELOPMENT:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_SCIENCE", 2, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_GROWTH", 2, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_PRODUCTION", 1, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_EXPANSION:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_EXPANSION", 1, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_SCIENCE", 2, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_GROWTH", 1, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_HAPPINESS", 1, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_MILITARISTIC_EXPANSION:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_OFFENSE", 3, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_DEFENSE", 1, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_MOBILE", 1, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_MILITARY:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_DEFENSE", 3, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_GOLD", 1, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_HAPPINESS_RECOVERY:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_HAPPINESS", 3, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_GROWTH", -2, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_TREASURY_RECOVERY:
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_GOLD", 3, iFlavorValue);
			AddDirectiveTechFlavorBias(eFlavor, "FLAVOR_TILE_IMPROVEMENT", 1, iFlavorValue);
			break;
		case PRIMARY_STRATEGY_BALANCED:
		case NO_PRIMARY_STRATEGY_DIRECTIVE:
		case NUM_PRIMARY_STRATEGY_DIRECTIVE_TYPES:
		default:
			break;
		}
...
void CvPlayerTechs::AddFlavorAsStrategies(int iPropagatePercent)
{
	...
	for(int iFlavor = 0; iFlavor < GC.getNumFlavorTypes(); iFlavor++)
	{
		...
		//MOD: bias tech flavors after vanilla current/personality blend
		iFlavorValue = GetDirectiveAdjustedTechFlavorValue(m_pPlayer, (FlavorTypes)iFlavor, iFlavorValue);
