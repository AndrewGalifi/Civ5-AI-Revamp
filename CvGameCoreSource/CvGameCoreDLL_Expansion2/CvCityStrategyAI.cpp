...
namespace
{
	//MOD: experiment city-strategy helpers
	void AddDirectiveFlavorBias(FlavorTypes eTargetFlavor, const char* szFlavorName, int iDelta, int& iFlavorValue)
	{
		const int iFlavorType = GC.getInfoTypeForString(szFlavorName, true);
		if(iDelta != 0 && iFlavorType != -1 && eTargetFlavor == (FlavorTypes)iFlavorType)
		{
			iFlavorValue += iDelta;
		}
	}

	struct DirectiveFlavorBias
	{
		StrategyDirectivePrimaryTypes m_ePrimaryStrategy;
		const char* m_szFlavorName;
		int m_iDelta;
	};

	void AddPrimaryDirectiveFlavorBiases(StrategyDirectivePrimaryTypes ePrimaryStrategy, FlavorTypes eFlavor, int& iFlavorValue)
	{
		static const DirectiveFlavorBias aDirectiveFlavorBiases[] =
		{
			{ PRIMARY_STRATEGY_DEVELOPMENT, "FLAVOR_GROWTH", 4 },
			{ PRIMARY_STRATEGY_DEVELOPMENT, "FLAVOR_PRODUCTION", 2 },
			{ PRIMARY_STRATEGY_DEVELOPMENT, "FLAVOR_EXPANSION", -4 },

			{ PRIMARY_STRATEGY_EXPANSION, "FLAVOR_EXPANSION", 4 },
			{ PRIMARY_STRATEGY_EXPANSION, "FLAVOR_GROWTH", 2 },

			{ PRIMARY_STRATEGY_MILITARISTIC_EXPANSION, "FLAVOR_OFFENSE", 6 },
			{ PRIMARY_STRATEGY_MILITARISTIC_EXPANSION, "FLAVOR_DEFENSE", 3 },
			{ PRIMARY_STRATEGY_MILITARISTIC_EXPANSION, "FLAVOR_EXPANSION", -3 },

			{ PRIMARY_STRATEGY_MILITARY, "FLAVOR_DEFENSE", 4 },
			{ PRIMARY_STRATEGY_MILITARY, "FLAVOR_OFFENSE", 3 },
			{ PRIMARY_STRATEGY_MILITARY, "FLAVOR_EXPANSION", -4 },
			{ PRIMARY_STRATEGY_MILITARY, "FLAVOR_GROWTH", -2 },

			{ PRIMARY_STRATEGY_HAPPINESS_RECOVERY, "FLAVOR_HAPPINESS", 8 },
			{ PRIMARY_STRATEGY_HAPPINESS_RECOVERY, "FLAVOR_EXPANSION", -8 },
			{ PRIMARY_STRATEGY_HAPPINESS_RECOVERY, "FLAVOR_GROWTH", -4 },

			{ PRIMARY_STRATEGY_TREASURY_RECOVERY, "FLAVOR_GOLD", 8 },
			{ PRIMARY_STRATEGY_TREASURY_RECOVERY, "FLAVOR_EXPANSION", -6 }
		};

		for(int iI = 0; iI < ARRAYSIZE(aDirectiveFlavorBiases); iI++)
		{
			if(aDirectiveFlavorBiases[iI].m_ePrimaryStrategy == ePrimaryStrategy)
			{
				AddDirectiveFlavorBias(eFlavor, aDirectiveFlavorBiases[iI].m_szFlavorName, aDirectiveFlavorBiases[iI].m_iDelta, iFlavorValue);
			}
		}
	}
	const int AI_EXPERIMENT_EARLY_LIBRARY_WEIGHT_PENALTY = 1500;
	const int AI_EXPERIMENT_WORK_BOAT_LUXURY_WEIGHT_BONUS = 15000;
	const int AI_EXPERIMENT_WORK_BOAT_RESOURCE_WEIGHT_BONUS = 6000;
	const int AI_EXPERIMENT_GRANARY_PRIORITY_END_TURN = 110;
	const int AI_EXPERIMENT_GRANARY_CAPITAL_PRIORITY_TURN = 85;
	...
	BuildingTypes GetExperimentBuildingForClass(CvPlayerAI& kPlayer, const char* szBuildingClass)
	{
		...
		return (BuildingTypes)kPlayer.getCivilizationInfo().getCivilizationBuildings(iBuildingClass);
	}

	bool IsExperimentMilitaryDirective(CvPlayerAI& kPlayer)
	{
	...
	bool IsExperimentMilitaryUnit(CvUnitEntry* pkUnitInfo)
	{
		if(pkUnitInfo == NULL)
		{
			return false;
		...
		switch((UnitAITypes)pkUnitInfo->GetDefaultUnitAIType())
		{
		case UNITAI_ATTACK:
	...
	int CountExperimentLandCombatUnitsAndBuilds(CvPlayerAI& kPlayer)
	{
		int iCount = 0;
		int iLoop = 0;
	...
	int CountExperimentRangedLandCombatUnitsAndBuilds(CvPlayerAI& kPlayer)
	{
		int iCount = 0;
		int iLoop = 0;
		for(CvUnit* pLoopUnit = kPlayer.firstUnit(&iLoop); pLoopUnit != NULL; pLoopUnit = kPlayer.nextUnit(&iLoop))
	...
	int GetExperimentMilitaryProductionTarget(CvPlayerAI& kPlayer)
	{
		if(!IsExperimentMilitaryDirective(kPlayer))
		{
			return 0;
		}
		...
		if(kDirective.m_bMilitaryProductionUrgent)
		{
			return (iNonPuppetCities * 2) + 1;
		}

		return iNonPuppetCities + 1;
	}

	bool IsExperimentMilitaryProductionDeficit(CvPlayerAI& kPlayer)
	{
		const int iTargetCombatUnits = GetExperimentMilitaryProductionTarget(kPlayer);
		return iTargetCombatUnits > 0 && CountExperimentLandCombatUnitsAndBuilds(kPlayer) < iTargetCombatUnits;
	}
	int GetExperimentRangedUnitWeight(CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo)
	{
		if(!IsExperimentMilitaryDirective(kPlayer) || !IsExperimentRangedLandCombatUnit(pkUnitInfo))
		{
			return 0;
		}

		const int iCombatUnits = CountExperimentLandCombatUnitsAndBuilds(kPlayer);
		if(iCombatUnits > 0 && CountExperimentRangedLandCombatUnitsAndBuilds(kPlayer) * StrategyDirectiveAIConstants::RANGED_TARGET_RATIO_DIVISOR < iCombatUnits)
		{
			return AI_EXPERIMENT_RANGED_UNIT_WEIGHT_BONUS;
		}

		return 0;
	}
	bool ShouldSuppressExperimentMilitaryUnit(CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo)
	{
		if(!ShouldUseStrategyDirectiveAI(kPlayer.GetID()) || !IsExperimentMilitaryUnit(pkUnitInfo))
		{
			return false;
		}
	...
	{
		int iWonderShots = kPlayer.GetNumWonders();

		int iLoop = 0;
		for(CvCity* pLoopCity = kPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kPlayer.nextCity(&iLoop))
		{
			if(pLoopCity == NULL || pLoopCity == pCurrentCity)
			{
				continue;
			}
			...
			const CvBuildingClassInfo& kBuildingClassInfo = pkProductionBuildingInfo->GetBuildingClassInfo();
	...
	int GetExperimentGlobalProductionWonderPercent(CvPlayerAI& kPlayer)
	{
		if(!ShouldUseStrategyDirectiveAI(kPlayer.GetID()))
		{
			return 100;
		...
		const int iPlayerProduction = kPlayer.calculateTotalYield(YIELD_PRODUCTION);
		...
		for(int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
		{
			...
			if(eLoopPlayer != kPlayer.GetID() && iLoopProduction > iPlayerProduction)
			{
				iProductionRank++;
			}
		}
	...
	int AdjustExperimentGlobalProductionWonderWeight(CvCity* pCity, CvPlayerAI& kPlayer, CvBuildingEntry* pkBuildingInfo, int iWeight)
	{
		...
		const int iProductionPercent = GetExperimentGlobalProductionWonderPercent(kPlayer);
	...
	int AdjustExperimentEarlyWonderWeight(CvCity* pCity, CvPlayerAI& kPlayer, CvBuildingEntry* pkBuildingInfo, int iWeight)
	{
		...
		if(GC.getGame().getGameTurn() > StrategyDirectiveAIConstants::EARLY_WONDER_DISINCENTIVE_TURN)
		{
			return iWeight;
		}
		...
		const int iWonderShots = CountExperimentEarlyWonderShots(kPlayer, pCity);
		const int iWeightPercent = (iWonderShots > 0) ? StrategyDirectiveAIConstants::EARLY_WONDER_FOLLOWUP_PERCENT : StrategyDirectiveAIConstants::EARLY_WONDER_FIRST_PERCENT;
		return max(1, (iWeight * iWeightPercent) / 100);
	}
	bool ShouldSuppressExperimentEarlyLibrary(CvCity* pCity, CvPlayerAI& kPlayer, BuildingTypes eBuilding)
	{
		if(pCity == NULL || !ShouldUseStrategyDirectiveAI(kPlayer.GetID()) || GC.getGame().getGameTurn() >= StrategyDirectiveAIConstants::SCIENCE_INFRASTRUCTURE_PRIORITY_TURN)
		{
			return false;
		}

		const BuildingTypes eLibrary = GetExperimentBuildingForClass(kPlayer, "BUILDINGCLASS_LIBRARY");
		return eBuilding == eLibrary;
	}
	int GetExperimentGranaryWeight(CvCity* pCity, CvPlayerAI& kPlayer, BuildingTypes eBuilding)
	{
		if(pCity == NULL || !ShouldUseStrategyDirectiveAI(kPlayer.GetID()) || GC.getGame().getGameTurn() > AI_EXPERIMENT_GRANARY_PRIORITY_END_TURN)
		{
			return 0;
		}

		const BuildingTypes eGranary = GetExperimentBuildingForClass(kPlayer, "BUILDINGCLASS_GRANARY");
		if(eGranary == NO_BUILDING || eBuilding != eGranary || pCity->GetCityBuildings()->GetNumBuilding(eGranary) > 0)
		{
			return 0;
		}

		if(pCity->isCapital() && GC.getGame().getGameTurn() < AI_EXPERIMENT_GRANARY_CAPITAL_PRIORITY_TURN)
		{
			return -AI_EXPERIMENT_CAPITAL_GRANARY_EARLY_WEIGHT_PENALTY;
		}
	...
	int GetExperimentWorkerWeight(CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo)
	{
		...
		if(pkUnitInfo->GetDefaultUnitAIType() != UNITAI_WORKER || pkUnitInfo->GetDomainType() != DOMAIN_LAND)
		{
			return 0;
		}
		...
		const int iTotalCities = kPlayer.getNumCities();
		if(iTotalCities <= 0 || GC.getGame().getGameTurn() <= StrategyDirectiveAIConstants::WORKER_BASE_RATIO_TURN)
		{
			return 0;
		}

		const int iTargetWorkers = (GC.getGame().getGameTurn() < StrategyDirectiveAIConstants::WORKER_FULL_RATIO_TURN) ? iTotalCities : ((iTotalCities * 3) + 1) / 2;
		const int iWorkersIncludingBuilds = kPlayer.GetNumUnitsWithUnitAI(UNITAI_WORKER, true, false);
		const int iWorkerDeficit = iTargetWorkers - iWorkersIncludingBuilds;
		if(iWorkerDeficit <= 0)
		{
			return 0;
		}

		return kDirective.m_iWorkerBaseWeightBonus + ((iWorkerDeficit - 1) * kDirective.m_iWorkerDeficitWeightBonus);
	}
	int GetExperimentWorkBoatWeight(CvCity* pCity, CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo)
	{
		if(pCity == NULL || pkUnitInfo == NULL || !ShouldUseStrategyDirectiveAI(kPlayer.GetID()))
		{
			return 0;
		}

		if(pkUnitInfo->GetDefaultUnitAIType() != UNITAI_WORKER_SEA)
		{
			return 0;
		}

		int iLuxuryResources = 0;
		int iOtherResources = 0;
		for(int iPlotLoop = 0; iPlotLoop < NUM_CITY_PLOTS; iPlotLoop++)
		{
			CvPlot* pLoopPlot = plotCity(pCity->getX(), pCity->getY(), iPlotLoop);
			if(pLoopPlot == NULL || pLoopPlot->getOwner() != kPlayer.GetID() || !pLoopPlot->isWater() || pLoopPlot->getImprovementType() != NO_IMPROVEMENT)
			{
				continue;
			}
			...
			const ResourceTypes eResource = pLoopPlot->getResourceType(kPlayer.getTeam());
		...
		const int iCityTargetWorkBoats = iLuxuryResources + iOtherResources;
		if(iCityTargetWorkBoats <= 0)
		{
			return 0;
		}

		int iEmpireTargetWorkBoats = 0;
		int iLoop = 0;
		for(CvCity* pLoopCity = kPlayer.firstCity(&iLoop); pLoopCity != NULL; pLoopCity = kPlayer.nextCity(&iLoop))
		{
			for(int iPlotLoop = 0; iPlotLoop < NUM_CITY_PLOTS; iPlotLoop++)
			{
				CvPlot* pLoopPlot = plotCity(pLoopCity->getX(), pLoopCity->getY(), iPlotLoop);
				if(pLoopPlot == NULL || pLoopPlot->getOwner() != kPlayer.GetID() || !pLoopPlot->isWater() || pLoopPlot->getImprovementType() != NO_IMPROVEMENT)
				{
					continue;
				}

				if(!pLoopCity->GetCityCitizens()->IsCanWork(pLoopPlot))
				{
					continue;
				}

				if(pLoopPlot->getResourceType(kPlayer.getTeam()) != NO_RESOURCE)
				{
					iEmpireTargetWorkBoats++;
				}
			}
		}

		const int iWorkBoatsIncludingBuilds = kPlayer.GetNumUnitsWithUnitAI(UNITAI_WORKER_SEA, true, true);
		if(iEmpireTargetWorkBoats <= iWorkBoatsIncludingBuilds)
		{
			return 0;
		}

		return (iLuxuryResources * AI_EXPERIMENT_WORK_BOAT_LUXURY_WEIGHT_BONUS) + (iOtherResources * AI_EXPERIMENT_WORK_BOAT_RESOURCE_WEIGHT_BONUS);
	}
	bool IsExperimentSettlerUnit(CvUnitEntry* pkUnitInfo)
	{
		return pkUnitInfo != NULL && pkUnitInfo->GetDefaultUnitAIType() == UNITAI_SETTLE;
	}

	int GetExperimentExpansionSettlerWeight(CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo)
	{
		if(pkUnitInfo == NULL || !ShouldUseStrategyDirectiveAI(kPlayer.GetID()))
		{
			return 0;
		...
		if(IsExperimentSettlerUnit(pkUnitInfo))
		{
			return kDirective.m_iSettlerWeightBonus;
		}

		return 0;
	}
	bool ShouldSuppressExperimentSettlerForAreaCap(CvPlayerAI& kPlayer)
	{
		if(!ShouldUseStrategyDirectiveAI(kPlayer.GetID()))
		{
			return false;
		...
		const int iSettlersIncludingBuilds = kPlayer.GetNumUnitsWithUnitAI(UNITAI_SETTLE, true, false);
		return iSettlersIncludingBuilds >= iNumGoodAreas;
	}

	bool ShouldSuppressExperimentSettlerBuild(CvPlayerAI& kPlayer, CvUnitEntry* pkUnitInfo, bool bEnoughSettlers)
	{
		if(!IsExperimentSettlerUnit(pkUnitInfo))
		{
			return false;
		...
		if(ShouldSuppressExperimentSettlerForAreaCap(kPlayer))
		{
			return true;
		}

		return bEnoughSettlers && GetExperimentExpansionSettlerWeight(kPlayer, pkUnitInfo) <= 0;
	}

	int GetDirectiveAdjustedFlavorValue(CvCity* pCity, FlavorTypes eFlavor, int iBaseFlavorValue)
	{
		if(pCity == NULL)
		{
		...
		AddDirectiveFlavorBias(eFlavor, "FLAVOR_CULTURE", AI_EXPERIMENT_BASE_CULTURE_FLAVOR_BONUS, iFlavorValue);

		AddPrimaryDirectiveFlavorBiases(kDirective.m_ePrimaryStrategy, eFlavor, iFlavorValue);

		if(kDirective.m_bLowHappiness)
		{
			AddDirectiveFlavorBias(eFlavor, "FLAVOR_HAPPINESS", 8, iFlavorValue);
			AddDirectiveFlavorBias(eFlavor, "FLAVOR_EXPANSION", -5, iFlavorValue);
			AddDirectiveFlavorBias(eFlavor, "FLAVOR_GROWTH", -3, iFlavorValue);
		}

		if(kDirective.m_bLowGold)
		{
			AddDirectiveFlavorBias(eFlavor, "FLAVOR_GOLD", 8, iFlavorValue);
			AddDirectiveFlavorBias(eFlavor, "FLAVOR_EXPANSION", -3, iFlavorValue);
		}
		return max(0, iFlavorValue);
	}
	//END MOD
}
...
void CvCityStrategyAI::FlavorUpdate()
{
	...
	{
		//MOD: bias city production flavors for experiment player before propagation
		int iFlavorValue = GetDirectiveAdjustedFlavorValue(m_pCity, (FlavorTypes)iFlavor, GetLatestFlavorValue((FlavorTypes)iFlavor));
...
void CvCityStrategyAI::ChooseProduction(bool bUseAsyncRandom, BuildingTypes eIgnoreBldg /* = NO_BUILDING */, UnitTypes eIgnoreUnit/*  = NO_UNIT */)
{
	...
	if(eUnitForOperation != NO_UNIT)
	{
		...
		//MOD: suppress settler operation requests when NC, area, or vanilla caps block expansion
		if(ShouldSuppressExperimentSettlerBuild(kPlayer, pkUnitEntry, bEnoughSettlers))
		{
			iTempWeight = 0;
		}
		//MOD: suppress combat-unit production unless a military directive is active
		if(ShouldSuppressExperimentMilitaryUnit(kPlayer, pkUnitEntry))
		{
			iTempWeight = 0;
		}
	...
	if(eUnitForArmy != NO_UNIT)
	{
		...
		//MOD: suppress vanilla army requests unless a military directive is active
		CvUnitEntry* pkArmyUnitEntry = GC.getUnitInfo(eUnitForArmy);
		if(ShouldSuppressExperimentMilitaryUnit(kPlayer, pkArmyUnitEntry))
		{
			iTempWeight = 0;
		}
	...
	for(iBldgLoop = 0; iBldgLoop < GC.GetGameBuildings()->GetNumBuildings(); iBldgLoop++)
	{
		...
		if(iBldgLoop != eIgnoreBldg && m_pCity->canConstruct(eLoopBuilding))
		{
			...
			//MOD: do not let early Libraries slip through on high base weight
			if(ShouldSuppressExperimentEarlyLibrary(GetCity(), kPlayer, eLoopBuilding))
			{
				iTempWeight = 0;
			}
			else
	...
	if(!GetCity()->IsPuppet())
	{
		...
		for(iUnitLoop = 0; iUnitLoop < GC.GetGameUnits()->GetNumUnits(); iUnitLoop++)
		{
			        ...
			        m_pCity->canTrain((UnitTypes)iUnitLoop))
			{
				...
				//MOD: prioritize workers below 1 per total city after the base-ratio turn, then 1.5 per total city from the full-ratio turn
