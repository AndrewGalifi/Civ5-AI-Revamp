...
namespace
{
	//MOD: early wonder specialization restraint.

	int CountExperimentActiveOrCompletedWonders(CvPlayer* pPlayer)
	{
		if(pPlayer == NULL)
		{
			return 0;
		...
		int iWonderShots = pPlayer->GetNumWonders();
		int iLoop = 0;
	...
	int AdjustExperimentEarlyWonderSpecializationWeight(CvPlayer* pPlayer, int iWonderWeight)
	{
		if(pPlayer == NULL || iWonderWeight <= 0 || !ShouldUseStrategyDirectiveAI(pPlayer->GetID()))
		{
		...
		const int iWonderShots = CountExperimentActiveOrCompletedWonders(pPlayer);
		...
		return max(1, (iWonderWeight * iWeightPercent) / 100);
	}
	//END MOD
}
//=====================================
...
int CvCitySpecializationAI::WeightProductionSubtypes(int iFlavorWonder, int iFlavorSpaceship)
{
	...
	//MOD: reduce early wonder-city specialization pressure without fully forbidding wonders.
	iWonderWeight = AdjustExperimentEarlyWonderSpecializationWeight(m_pPlayer, iWonderWeight);
