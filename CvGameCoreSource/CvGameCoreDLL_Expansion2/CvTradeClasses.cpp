...
//MOD: internal food trade-route preference helpers.
namespace
{

	bool IsExperimentInternalFoodTradePriority(CvPlayer* pPlayer)
	{
		return pPlayer != NULL && ShouldUseStrategyDirectiveAI(pPlayer->GetID()) && pPlayer->getNumCities() > 1 && pPlayer->GetHappiness() >= 0 && GC.getGame().getGameTurn() <= StrategyDirectiveAIConstants::INTERNAL_FOOD_TRADE_PRIORITY_END_TURN;
	}

	int GetExperimentTradeRouteDanger(CvPlayer* pPlayer, const TradeConnection& kTradeConnection)
	{
		if(pPlayer == NULL)
		{
			return MAX_INT;
		...
		for(uint uiPlotList = 0; uiPlotList < kTradeConnection.m_aPlotList.size(); uiPlotList++)
		{
			...
			int iDangerValue = pPlayer->GetPlotDanger(*pPlot);
			if(iDangerValue == 0)
			{
				if(!pPlot->isVisible(pPlayer->getTeam()))
				{
					iDangerValue += 1;
				}

				if(pPlot->getTeam() != NO_TEAM && GET_TEAM(pPlayer->getTeam()).isAtWar(pPlot->getTeam()))
				{
					iDangerValue += 1000;
				}
			}
	...
	int ScoreExperimentFoodTR(CvPlayer* pPlayer, const TradeConnection& kTradeConnection)
	{
		if(pPlayer == NULL || kTradeConnection.m_eConnectionType != TRADE_CONNECTION_FOOD || kTradeConnection.m_eOriginOwner != pPlayer->GetID() || kTradeConnection.m_eDestOwner != pPlayer->GetID())
		{
			return 0;
		}

		if(pPlayer->GetTrade()->CheckTradeConnectionWasPlundered(kTradeConnection))
		{
			return 0;
		}
		...
		const int iFoodTimes100 = pPlayer->GetTrade()->GetTradeConnectionValueTimes100(kTradeConnection, YIELD_FOOD, true);
		int iScore = 1000 + iFoodTimes100;
		iScore += max(0, 20 - pDestCity->getPopulation()) * 25;
		iScore -= (int)kTradeConnection.m_aPlotList.size() * 5;
		iScore -= GetExperimentTradeRouteDanger(pPlayer, kTradeConnection) * 50;
...
{	
	const bool bExperimentInternalFoodPriority = IsExperimentInternalFoodTradePriority(m_pPlayer);
	...
	{
		if(bExperimentInternalFoodPriority)
		{
			//MOD: prefer internal food early, roughly capital first, best expand second, capital again if possible.
			CvCity* pCapital = m_pPlayer->getCapitalCity();
			std::vector<TRSortElement> aCapitalFoodSortedTR;
			std::vector<TRSortElement> aExpandFoodSortedTR;

			for (uint ui = 0; ui < aTradeConnectionList.size(); ui++)
			{
				TRSortElement kElement;
				kElement.m_kTradeConnection = aTradeConnectionList[ui];
				kElement.m_iScore = ScoreExperimentFoodTR(m_pPlayer, aTradeConnectionList[ui]);
				if (kElement.m_iScore <= 0)
				{
					continue;
				}

				CvCity* pDestCity = CvGameTrade::GetDestCity(aTradeConnectionList[ui]);
				if(pCapital != NULL && pDestCity == pCapital)
				{
					aCapitalFoodSortedTR.push_back(kElement);
				}
				else
				{
					aExpandFoodSortedTR.push_back(kElement);
				}
			}

			std::stable_sort(aCapitalFoodSortedTR.begin(), aCapitalFoodSortedTR.end(), SortTRDescending());
			std::stable_sort(aExpandFoodSortedTR.begin(), aExpandFoodSortedTR.end(), SortTRDescending());

			aFoodSortedTR.clear();
			if(aCapitalFoodSortedTR.size() > 0)
			{
				aFoodSortedTR.push_back(aCapitalFoodSortedTR[0]);
			}
			if(aExpandFoodSortedTR.size() > 0)
			{
				aFoodSortedTR.push_back(aExpandFoodSortedTR[0]);
			}
			if(aCapitalFoodSortedTR.size() > 1)
			{
				aFoodSortedTR.push_back(aCapitalFoodSortedTR[1]);
			}
			for(uint ui = 2; ui < aCapitalFoodSortedTR.size(); ui++)
			{
				aFoodSortedTR.push_back(aCapitalFoodSortedTR[ui]);
			}
			for(uint ui = 1; ui < aExpandFoodSortedTR.size(); ui++)
			{
				aFoodSortedTR.push_back(aExpandFoodSortedTR[ui]);
			}
		}
		else
		{
	...
	if(bExperimentInternalFoodPriority)
	{
		//MOD: put early/mid internal food routes before vanilla gold-route preference.
		uint uiSeaFoodTradesAdded = 0;
		uint uiLandFoodTradesAdded = 0;

		for (uint ui = 0; ui < aFoodSortedTR.size(); ui++)
		{
			if(aFoodSortedTR[ui].m_kTradeConnection.m_eDomain == DOMAIN_SEA && uiSeaFoodTradesAdded < StrategyDirectiveAIConstants::INTERNAL_FOOD_TRADE_MAX_ROUTES_PER_DOMAIN)
			{
				aTradeConnectionList.push_back(aFoodSortedTR[ui].m_kTradeConnection);
				uiSeaFoodTradesAdded++;
			}
		}

		for (uint ui = 0; ui < aFoodSortedTR.size(); ui++)
		{
			if(aFoodSortedTR[ui].m_kTradeConnection.m_eDomain == DOMAIN_LAND && uiLandFoodTradesAdded < StrategyDirectiveAIConstants::INTERNAL_FOOD_TRADE_MAX_ROUTES_PER_DOMAIN)
			{
				aTradeConnectionList.push_back(aFoodSortedTR[ui].m_kTradeConnection);
				uiLandFoodTradesAdded++;
			}
		}
