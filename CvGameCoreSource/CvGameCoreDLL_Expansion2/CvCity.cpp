			...
			if(GET_TEAM(getTeam()).GetTeamTechs()->HasTech((TechTypes)pkResource->getTechReveal()))
			{
				...
				if (iRevealPolicy == NO_POLICY || GET_PLAYER(getOwner()).GetPlayerPolicies()->HasPolicy((PolicyTypes)iRevealPolicy))
				{
					...
					else if(eResourceUsage == RESOURCEUSAGE_LUXURY)
					{
						...
						//MOD: aggressively buy adjacent unowned unique luxuries for the experiment player.
						if(ShouldUseStrategyDirectiveAI(getOwner()) && GET_PLAYER(getOwner()).getNumResourceTotal(eResource, false) == 0 && pPlot->getOwner() == NO_PLAYER)
						{
							for(int iDirectionLoop = 0; iDirectionLoop < NUM_DIRECTION_TYPES; iDirectionLoop++)
							{
								const CvPlot* pAdjacentPlot = plotDirection(pPlot->getX(), pPlot->getY(), ((DirectionTypes)iDirectionLoop));
