...
bool CvTacticalAI::IsHighPriorityCivilianTarget(CvTacticalTarget* pTarget)
{
	...
	if(pUnit)
	{
		...
		if(pkUnitInfo)
		{
			...
			//MOD: prioritize reclaiming free barbarian workers for the experiment player.
			if(!bRtnValue && ShouldUseStrategyDirectiveAI(m_pPlayer->GetID()) && pUnit->getOwner() == BARBARIAN_PLAYER && pUnit->AI_getUnitAIType() == UNITAI_WORKER)
			{
				bRtnValue = true;
			}
